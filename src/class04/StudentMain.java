...
public class StudentMain {
	...
	public static void main(String[] args) {
		...
		// new는 객체 생성하는 요소 , StudentData : 생성자 호출
		StudentData sd = new StudentData();	 
		Student[] students = sd.students; // 위에서 생성된 StudentData 객체의 필드 접근
	...
	private static void printStudent(Student[] students) {
		...
		for(int i=0; i<students.length; i++) {
			System.out.printf("%s      %s       %s      %d      %s       %d \n", 
					students[i].getId(), students[i].getJumin(), students[i].getName(), 
		...
		for (Student student : students) {
			System.out.printf("%s      %s       %s      %d      %s       %d \n",
					student.getId() , student.getJumin() , student.getName() ,
